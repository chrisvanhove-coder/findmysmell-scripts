...
var FMS_ARCHETYPES = {
  'CEO': {
    ...
    tagline:  'Propre. Architectural. Concentré.',
    desc:     "Vous n'avez pas besoin d'une autre recommandation. Vous avez besoin de quelque chose qui fonctionne. Pas de douceur. Pas d'histoire. Pas de « comment vous sentez-vous ? »",
...
'JAPAN': {
    headline: 'VOUS ÊTES\nINSISTAMMENT\nCALME.',
    tagline:  'Minimal. Intentionnel. Au-dessus du bruit.',
    desc:     "Vous parlez moins. Vous choisissez mieux. Vous vous déplacez avec intention. Calme, parce que vous n'avez rien à prouver.",
...
'HUG': {
    headline: 'VOUS ÊTES\nUN CÂLIN.',
    tagline:  'Ouvert. Présent. Discrètement rassurant.',
    desc:     "Vous écoutez sans résoudre. Vous restez sans juger. La chaleur, pour vous, n'est pas une performance.",
...
'OFFGRID': {
    headline: 'VOUS PLANIFIEZ\nSECRÈTEMENT\nDE DISPARAÎTRE.',
    tagline:  'Brut. Ancré. Présent.',
    desc:     "Vous ne fantasmez pas sur l'évasion. Vous vous y préparez. Vous préférez le mouvement à la conversation. Le silence aux explications. Pas besoin de confort. Besoin d'espace.",
...
'OUTOFTIME': {
    headline: 'VOUS ÊTES\nCHRONIQUEMENT\nAILLEURS.',
    tagline:  'Introverti. Mystérieux. Entièrement vous-même.',
    desc:     "Vous n'êtes pas en retard sur le monde. Vous en êtes légèrement à l'écart. Vous laissez les choses se révéler.",
...
'SUMMER': {
    headline: 'VOUS ÊTES\nPATHOLOGIQUEMENT\nDÉTENDU(E).',
    tagline:  'Léger. En mouvement. Confiant.',
    desc:     "Vous passez facilement d'une personne à l'autre, d'un plan à l'autre. Vous faites confiance au processus et gardez les choses légères.",
...
'THERAPIST': {
    headline: 'VOUS ÊTES\nDANGEREUSEMENT\nEMPATHIQUE.',
    tagline:  'Présent. Intentionnel. Sélectif.',
    desc:     "Vous comprenez les gens rapidement — parce que vous remarquez ce qui compte vraiment. Vous n'êtes pas pour tout le monde. Et vous n'essayez pas de l'être.",
...
function fmsShareText() {
  ...
  return a ? a.headline.replace('\n', ' ') + '\n' + a.tagline + '\n\nDécouvrez votre archétype olfactif → ' + url : url;
}
...
function fmsDrawBase(ctx, arch, S, PAD) {
  ...
  ctx.fillText('FIND MY SMELL  ·  Quiz de Personnalité Olfactive', PAD, 78);
  ctx.globalAlpha = 1;
...
function fmsDrawFooter(ctx, arch, S, PAD) {
  ...
  ctx.fillText(' découvrez le vôtre sur  ·  findmysmell.com ', PAD, S - 44);
  ctx.globalAlpha = 1;
}
...
function fmsDrawCard(canvas, callback) {
  ...
  function finishCard() {
    ...
    ctx.fillText('PARFUM RECOMMANDÉ', PAD, perfY + 6); ctx.globalAlpha = 1;
...
function fmsDrawCard2(canvas, callback) {
  ...
  fmsLoadImages(allSrcs, function(images) {
    ...
    ctx.font = '400 22px Inconsolata,monospace';
    ctx.fillStyle = arch.accent; ctx.globalAlpha = 0.55;
    ctx.fillText('VOTRE PARFUM  ·  02 / 03', PAD, 136); ctx.globalAlpha = 1;
...
function fmsDrawCard3(canvas, callback) {
  ...
  fmsLoadImages(imgSrcs, function(images) {
    ...
    ctx.font = '400 22px Inconsolata,monospace';
    ctx.fillStyle = arch.accent; ctx.globalAlpha = 0.55;
    ctx.fillText('INGRÉDIENTS À DÉCOUVRIR  ·  03 / 03', PAD, 136); ctx.globalAlpha = 1;
