...
export const submissions = pgTable(
  'submissions',
  {
    ...
    /* Согласие на использование анонимных ответов в исследовании.
       У БРОШЕННОГО ПРОХОЖДЕНИЯ здесь всегда false, и это НЕ «человек
       отказался»: галочку показывают на последнем экране, и до неё он не
       дошёл. Отличать по `completed`: false + не завершено = не спрашивали.
       На использование это не влияет — нет согласия, значит в исследование
       такая запись не идёт, ровно как отказ. */
    consentResearch: boolean('consent_research').notNull().default(false),
    /* Дошёл ли человек до конца. false — прохождение брошено на полпути и
       записано тем, что успело накопиться: заказчица попросила видеть не
       только НА КАКОМ вопросе уходят (это и так есть в воронке), но и ЧТО
       успели ответить.
       Считается на сервере из самих ответов (missingQuestions), клиенту это
       поле не доверяется.
       Почему default true: колонка появилась, когда незавершённых записей
       ещё не существовало — прежний API отклонял их, а перенесённые из
       старой таблицы прохождения все с результатом. */
...
export const subscribers = pgTable('subscribers', {
  ...
  /* Когда письмо ушло ОТСЮДА, с этого сайта.
     ВНИМАНИЕ ТОМУ, КТО БУДЕТ ПИСАТЬ РАССЫЛКУ. У перенесённых из старой
     таблицы подписчиков здесь НЕ null, как можно подумать. Проверено в
     проде 23.09.2026: у всех девяти дата совпадает с их же `consent_at` до
     минуты — старый импорт переносил дату из самой таблицы, то есть момент,
     когда результат отправил старый сайт на Webflow. Как метка «второй раз
     не писать» это работает, но отправлял их не этот сайт.
     А НОВЫЙ импорт (src/lib/sheet-write.ts) пишет сюда null: выдумывать дату
     доставки перестали. После следующего импорта в таблице окажутся оба вида
     строк, и `sent_at is null` само по себе перестанет что-либо значить.
     Отличать по `consent_at`: у исторических он заметно раньше запуска нового
     сайта (12.09.2026). */
