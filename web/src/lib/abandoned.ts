 ...
 *
 * ВТОРОЙ ВЫЗЫВАЮЩИЙ — СТРАНИЦА РЕЗУЛЬТАТА. RecordSubmission вешает тот же
 * watchForAbandon как страховку: если обычная отправка не прошла ни с одной
 * попытки, маячок на уходе со страницы спасает полный набор ответов. Там
 * согласие уже спрошено, и уходит настоящее, а не false; набор полный, так
 * что сервер посчитает `completed = true`. Условие `wasSent()` ниже следит,
 * чтобы у благополучно сохранённого прохождения маячок не отправлялся вовсе.
 */
